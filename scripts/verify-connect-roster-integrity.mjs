...
const checks = [
  ...
  [route.includes("assertSwapBeforeCutoff(requester, partner") && route.includes("isRosterChangePastDeadline") && route.includes("loadRosterChangeDeadlineHour") && route.includes("roster_change_deadline_hour"), "swap cutoff uses HR-configured previous-day deadline hour"],
  [component.includes("rosterChangeDeadlineShortLabel") && component.includes('"No valid swap"') && component.includes("Only different rosters can be exchanged."), "unavailable swap days explain the eligibility and day-before lock rules"],
