-- Finance-only daily reporting; no changes to operational facts or import pipelines.
create function public.finance_report_count(p_normalized jsonb, p_raw jsonb, p_key text, p_aliases text[])
...
create function public.finance_business_daily_snapshot(p_company uuid, p_from date, p_through date, p_station_codes text[])
  ...
  return result || jsonb_build_object(
    'daily_shipments',coalesce((
      with current_source as materialized (
        select * from public.cps_shipment_daily
        ...
        from current_source c join public.report_import_rows r
          on r.company_id=p_company and r.source_type='amazon_shipments'
          and r.batch_id=c.source_batch_id and r.station_code=c.station_code
