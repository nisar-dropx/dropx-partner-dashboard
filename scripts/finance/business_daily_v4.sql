-- Separate SWA from MG and XPT variable billing: SWA has its own rates.
  ...
  return result || jsonb_build_object(
    'daily_shipments',coalesce((
      with current_source as materialized (
        select id,source_batch_id,station_code,work_date,provider_employee_id,client,total_delivery,amazon_delivery,swa_delivery,mfn,updated_at from public.cps_shipment_daily
      ...
      select jsonb_agg(to_jsonb(d) order by d.station_code,d.client,d.work_date) from (
        select c.station_code,c.client,c.work_date,
          ...
          case when count(*) filter(where c.amazon_delivery is null)=0 then sum(c.amazon_delivery)::text end as mg_deliveries,
          case when count(*) filter(where c.swa_delivery is null)=0 then sum(c.swa_delivery)::text end as swa,
