...
const checks = [
  ...
  [performanceApi.includes('clean(department.data?.code).toUpperCase() === "OPS"') && performanceApi.includes("operationsAssignment || companyOwner") && performanceApi.includes("hasConnectCompanyOwnerAccess(db(), account.companyId, engagement.data.person_id)") && performanceApi.includes("context.operationsEligible") && performanceApi.includes(": Promise.resolve(null)"), "operational scorecard and CPS respect the Operations department or verified company-owner access"],
