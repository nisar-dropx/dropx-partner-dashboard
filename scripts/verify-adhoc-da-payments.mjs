...
create table workforce(id uuid primary key,company_id uuid,source_profile_id uuid,deleted_at timestamptz,migration_state text default 'native');
...
insert into workforce(id,company_id) values('${worker}','${company}');
...
await assert.rejects(()=>insert({adhoc_work_date:'2026-09-11'}),/Select a DA/);
await assert.rejects(()=>insert({location_id:actor}),/Select a DA/);
