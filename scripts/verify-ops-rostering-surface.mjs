...
const checks = [
  ...
  [actions.includes("const currentWeek = rosterMonday(indiaToday())") && actions.includes("requestedStart < currentWeek ? currentWeek : requestedStart"), "OpsPulse roster corrections must never start earlier than the current week, matching People — a viewed future week may anchor the draft, but the current week is always the floor."],
  [actions.includes("seedOpsRecurringDraftEntries") && actions.includes('roster_kind: "recurring_weekly"') && actions.includes("Save and submit to replace the station's recurring roster"), "Station roster edits must create a replacement recurring weekly pattern instead of a one-week dated overlay."],
  [planner.includes("templateStart: templateStartRef.current") && planner.includes("setLiveRecurring(preparedIsRecurring)"), "The planner must preserve weekday-template edits when projecting the replacement roster into future weeks."],
