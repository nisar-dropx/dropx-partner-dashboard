...
- Pending, approved, processing, returned, cancelled or rejected payments do not create a deduction. `processed` / `paid` creates one approved `cash_recovery` adjustment, using the bank transaction amount (`amount`, falling back to `amount_requested`, as in bank export/reconciliation) and reference `OPS-ADHOC-DA:<payment UUID>`.
