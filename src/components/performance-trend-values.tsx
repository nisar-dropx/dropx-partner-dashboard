...
export function PerformanceTrendValues({ series, days }: { series: TrendSeries; days: 7 | 14 }) {
  ...
  return <>
    <div className="review-history-totals">
      ...
      {total != null ? <div><small>{days}-day recorded total</small><strong>{formatTrendValue(total, series.unit)}</strong></div> : null}
    </div>
    <div className="review-history-caption"><span>Daily values · newest first</span><span>{recorded.length}/{days} days recorded</span></div>
    <div className="review-history-values" tabIndex={0} role="region" aria-label={`${days}-day daily values for ${series.label}`}>
      <table>
