...
export function WorkforcePayoutTable({ rows }: { rows: WorkforcePayoutRow[] }) {
  ...
  function exportRows() {
    ...
    const columns = ["DropX ID","Registered Worker","Provider Member","Provider Member ID","Location Code","Provider","Model","Payment Method",...productionHeaders,"Base Amount","Additional Payments","Deductions","Net Pay","Status"];
  ...
  return <>
    ...
    <div className="table-wrap"><table className="workforce-payout-table workforce-payout-detail-table"><thead><tr><th rowSpan={2}>DropX ID</th><th rowSpan={2}>Registered Worker</th><th rowSpan={2}>Provider Member</th><th rowSpan={2}>Location Code</th><th rowSpan={2}>Provider / Model</th><th rowSpan={2}>Payment Method</th>{productionColumns.map((column) => <th className="production-group" colSpan={3} key={column.code}>{column.label}</th>)}<th rowSpan={2}>Base Amount</th><th rowSpan={2}>Additional Payments</th><th rowSpan={2}>Deductions</th><th rowSpan={2}>Net Pay</th><th rowSpan={2}>Status</th><th rowSpan={2}>Action</th></tr><tr>{productionColumns.flatMap((column) => [<th key={`${column.code}-count`}>Count</th>,<th key={`${column.code}-rate`}>Rate</th>,<th key={`${column.code}-amount`}>Amount</th>])}</tr></thead>
      <tbody>{visible.length ? visible.map((row) => <tr key={row.id}><td><strong>{row.dropxId}</strong></td><td><strong>{row.name}</strong></td><td><strong>{row.providerMemberName}</strong><small>{row.providerMemberId}</small></td><td><strong>{row.location}</strong></td><td>{row.provider}<small>{row.model}</small></td><td>{row.paymentMethod}</td>{productionColumns.flatMap((column) => { const item = row.productionBreakdown.find((value) => value.code === column.code); return [<td key={`${column.code}-count`}>{(item?.count ?? 0).toLocaleString("en-IN")}</td>,<td key={`${column.code}-rate`}>{money(item?.rate ?? 0)}</td>,<td key={`${column.code}-amount`}><strong>{money(item?.amount ?? 0)}</strong></td>]; })}<td>{money(row.baseAmount)}</td><td className="positive">+ {money(row.additions)}</td><td className="negative">- {money(row.deductions)}</td><td><strong>{money(row.netAmount)}</strong></td><td><span className="status-pill warn">{row.status}</span></td><td><button className="button secondary compact" type="button">Review</button></td></tr>) : <tr><td className="empty-cell" colSpan={12 + productionColumns.length * 3}>No mapped workforce payouts match the selected period and filters.</td></tr>}</tbody>
    </table></div>
