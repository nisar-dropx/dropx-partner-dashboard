...
export function DigestSettingsForm({control,action,canEdit,receipts}:{control:Control;action:(form:FormData)=>Promise<void>;canEdit:boolean;receipts:Receipt[]}){
 ...
 <fieldset className="form-grid" disabled={!canEdit} style={{border:0,padding:0,minWidth:0,width:'100%',gap:18,gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,280px),1fr))'}}>
 ...
 <label>Send every day at · IST<input className="field" name="schedule_time" type="time" required defaultValue={String(control.config.schedule_time||'08:00')}/></label>
 ...
 <label style={{gridColumn:'1 / -1'}}>Monthly subject<input className="field" name="subject" required maxLength={250} defaultValue={control.subject_template||''}/><small>Keep {'{{month}}'} and {'{{year}}'} in the subject. Replies retain the first subject used that month.</small></label>
 <label style={{gridColumn:'1 / -1'}}>Manager reminder<textarea className="field textarea" name="reminder" rows={3} maxLength={1000} defaultValue={String(control.config.manager_reminder||'')}/></label>
 {(['included_models','recipient_role_codes','location_role_codes'] as const).filter(key=>Array.isArray(control.config[key])).map(key=><label key={key}>{key==='included_models'?'Included operation model codes':key==='recipient_role_codes'?'Recipient designation codes':'Location mailbox team designation codes'}<input className="field" name={key} required defaultValue={(control.config[key] as string[]).join(', ')}/><small>Comma-separated master codes. Personal manager emails always retain their authorized reporting / location scope.</small></label>)}
 <label className="checkbox-row"><input type="checkbox" name="send_zero_cases" defaultChecked={control.config.send_zero_cases===true}/> Also send when there are no pending people / locations</label>
