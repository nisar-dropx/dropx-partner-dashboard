...
export function PerformanceNoonEmdEntry({entry,date,stationCode,canEdit,error}:{entry:PerformanceNoonEmd|null;date:string;stationCode:string;canEdit:boolean;error:string|null}) {
  return <section className="review-noon-emd" aria-label="EMD at 12 p.m.">
    <div><h3>EMD at 12 p.m. (%)</h3><p>Station total · {date.split("-").reverse().join("/")}</p></div>
