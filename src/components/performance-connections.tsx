...
function ConnectionForm({
  ...
  return (
    <ReviewActionForm action={savePerformanceConnection} className="review-connection-form" onSaved={onSaved}>
...
export function PerformanceConnections({
  ...
  const [adding, setAdding] = useState(canEdit && connections.length === 0);

  return (
    <section className="review-connections" aria-label="Station connections">
      <header>
        <div>
          <h3>
            Station connections <small>{connections.length}</small>
          </h3>
          <p>Arrival → unloading → clearance · each vehicle separately</p>
        </div>
        {canEdit ? (
          <button type="button" className="button secondary" onClick={() => setAdding(!adding)}>
            {adding ? "Cancel" : "+ Connection"}
          </button>
        ) : (
          <span className="review-view-only">View only</span>
        )}
      </header>

      {connections.length ? (
        <div className="review-connection-list">
          {connections.map((connection, index) => (
            <details key={connection.id} open={canEdit && index === 0}>
              <summary>
                <strong>{connection.label}</strong>
                <span>
                  <small>Arrival</small>
                  {displayTime(connection.arrival_at, date)}
                </span>
                <span>
                  <small>Unloaded</small>
                  {displayTime(connection.unloading_at, date)}
                </span>
                <span>
                  <small>Cleared</small>
                  {displayTime(connection.clearance_at, date)}
                </span>
              ...
              <p className="review-connection-audit">
                ...
                {new Intl.DateTimeFormat("en-IN", {
                  ...
                  hour: "2-digit",
                  minute: "2-digit"
                }).format(new Date(connection.updated_at))}
              </p>
            </details>
          ))}
        </div>
        ...
        <p className="review-empty">
          No connections recorded for this date.
          {canEdit ? " Enter vehicle arrival, unloading and clearance below." : " The station team can add timings here."}
        </p>
      )}
