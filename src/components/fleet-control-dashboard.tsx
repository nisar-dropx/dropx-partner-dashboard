        ...
        <div className="fc-content">
          ...
          {section === "overview" ? <>
            ...
            <section className="fc-kpis">
              ...
              <article><span className="purple"><Activity size={19} /></span><div><small>Ad-hoc vans today</small><strong>{todayAdHoc.length}</strong><p>{money(todayAdHoc.reduce((sum, row) => sum + row.amount, 0))} requested</p></div><b>Today</b></article>
            </section>
          ...
          {section === "approvals" ? <section className="fc-section">
            <div className="fc-section-head"><div><span className="fc-eyebrow">Vehicle payments</span><h1>Approval desk</h1><p>Approve station vehicle expenses and keep a complete decision trail.</p></div><button className="fc-button secondary" onClick={() => changeSection("reports")} type="button"><BarChart3 size={16} /> Payment report</button></div>
            <div className="fc-approval-layout">
              <div className="fc-panel fc-queue">
                ...
                {!filteredPayments.length ? <div className="fc-empty"><CircleDollarSign size={32} /><strong>No matching payment</strong><p>Vehicle payment requests will appear here.</p></div> : null}
              </div>
          ...
          {section === "adhoc" ? <section className="fc-section">
            <div className="fc-section-head"><div><span className="fc-eyebrow">Operations demand</span><h1>Day-level ad-hoc vans</h1><p>Station requests and paid cashbook entries, presented without CPS accounting menus.</p></div><label className="fc-date-filter"><CalendarDays size={16} /><input max={data.today} onChange={(event) => setAdHocDate(event.target.value)} type="date" value={adHocDate} /></label></div>
            <div className="fc-ad-hoc-summary"><article><small>Jobs on {date(adHocDate, false)}</small><strong>{filteredAdHoc.length}</strong></article><article><small>Requested amount</small><strong>{money(filteredAdHoc.reduce((sum, row) => sum + row.amount, 0))}</strong></article><article><small>Stations using ad-hoc vans</small><strong>{new Set(filteredAdHoc.map((row) => row.stationCode)).size}</strong></article><div><span><Wrench size={16} /> Breakdown-ready workflow</span><p>When “Vehicle breakdown” is selected as the reason, the request model is ready to require an affected fleet vehicle. Activation is deliberately deferred.</p></div></div>
