  ...
  for (const summary of summaries) {
    ...
    for (const { table, rows } of indexed) {
      ...
      if (table.name === "Performance scorecard") {
        ...
        paragraph("Time | Day start | EDD total | At station | On road | Delivered | Attempted/rejected/held | Inbound/returns", 8, muted);
        selected.forEach(row => {
          const keys = ["Checkpoint IST", "Day start EDD", "EDD total", "At station (source)", "On road (source)", "Delivered (source)", "Attempted rejected held (source)", "Inbound returns (source)"];
          paragraph(keys.map(k => row[k] ?? "—").join(" | ") + ` · ${row.Coverage} · ${row["Observed IST"] || "No observation"}`, 8);
          if (Number(row.Unverified) > 0 || Number(row.Other) > 0) paragraph(`Coverage note: ${row.Unverified || 0} need history/station reconciliation. Source detail: ${row["Source status breakdown"] || "Not recorded"}.`, 7, muted);
        });
