...
export function buildReviewReport(from: string, to: string, stations: ReportStation[], data: ReviewReportSources, targets: PerformanceTarget[], now = new Date()): ReviewReport {
  ...
  for (const date of reviewReportDates(from, to).reverse()) for (const station of [...stations].sort((a, b) => a.station_code.localeCompare(b.station_code))) {
    ...
    if (points.length) for (const row of timeline.rows) add(edd, { "Checkpoint IST": row.label, "Day start EDD": row.dayStart, "At station pending": row.point?.counts.todayAtStation ?? null, "On road": row.point?.counts.todayOnRoad ?? null, Delivered: row.point?.counts.todayDelivered ?? null, HFR: row.point?.counts.todayHfr ?? null, Attempted: row.point?.counts.todayAttempted ?? null, Unverified: row.point?.counts.todayUnverified ?? null, Other: row.point?.counts.todayOther ?? null, Coverage: row.state, "Observed IST": reportIst(row.point?.observedAt) });
    const current = stepRows.find(s => s.step_order === review?.current_step_order && s.status === "pending");
    add(summary, { "Station name": station.station_name || station.station_code, Cluster: station.cluster || "Unassigned", "Review status": !review ? "Not started" : review.status === "closed" ? "Completed" : "In progress", "Current reviewer": current ? `${current.reviewer_name} · ${current.reviewer_role}` : "", "Performance misses": fact ? misses : null, "Saved RCA or reasons": items.filter(i => text(i.root_cause).trim()).length, "Missing performance RCA": fact ? missing : null, "Open actions": followups.filter(f => f.status !== "done").length + items.filter(i => !isDisciplineRcaKey(i.metric_key) && i.status !== "done" && text(i.corrective_action).trim()).length, "At station EDD cleared time": timeline.summary, "Latest EDD pending": timeline.latest?.counts.hasSnapshot ? timeline.latest.counts.todayAtStation : null, "EDD observation IST": reportIst(timeline.latest?.observedAt), "EMD at noon %": number(emd?.emd_noon_pct), "Review takeaway": text(review?.review_summary), "Started IST": reportIst(review?.started_at), "Closed IST": reportIst(review?.closed_at), "Last updated IST": reportIst(review?.updated_at), "Source coverage": [!fact && "Scorecard not uploaded", !review && "Review not started", !points.length && "EDD history not recorded", !cost && "Cost source not loaded"].filter(Boolean).join("; ") || "Review, scorecard, EDD and cost sources available" });
  }
