...
export function buildReviewReport(from: string, to: string, stations: ReportStation[], data: ReviewReportSources, targets: PerformanceTarget[], now = new Date()): ReviewReport {
  ...
  const edd = table("EDD checkpoints", ["Checkpoint IST", "Day start EDD", "EDD total", "At station (source)", "On road (source)", "Delivered (source)", "Attempted rejected held (source)", "Inbound returns (source)", "Source status breakdown", "At station pending", "On road", "Delivered", "HFR", "HCR", "Observed INDUCTED / RECEIVED", "Attempted", "Unverified", "Other", "Coverage", "Observed IST"]);
  ...
  for (const date of reviewReportDates(from, to).reverse()) for (const station of [...stations].sort((a, b) => a.station_code.localeCompare(b.station_code))) {
    ...
    if (points.length) for (const row of timeline.rows) {
      ...
      add(edd, { "Checkpoint IST": row.label, "Day start EDD": timeline.dayStartAll, "EDD total": counts?.todayTotal ?? null,
        "At station (source)": counts?.movement?.atStation ?? counts?.todayObservedAtStation ?? null,
        "On road (source)": counts?.movement?.onRoad ?? null, "Delivered (source)": counts?.movement?.delivered ?? null,
        "Attempted rejected held (source)": counts?.movement?.exceptions ?? null, "Inbound returns (source)": counts?.movement?.transit ?? null,
        "Source status breakdown": counts?.movement?.statuses.map(s => `${s.status}: ${s.count}`).join("; ") ?? "Not recorded",
        "At station pending": counts?.todayAtStation ?? null, "On road": counts?.todayOnRoad ?? null, Delivered: counts?.todayDelivered ?? null, HFR: counts?.todayHfr ?? null, HCR: counts?.todayHcr ?? null, "Observed INDUCTED / RECEIVED": counts?.todayObservedAtStation ?? null, Attempted: counts?.todayAttempted ?? null, Unverified: counts?.todayUnverified ?? null, Other: counts?.todayOther ?? null, Coverage: row.state, "Observed IST": reportIst(row.point?.observedAt) });
    }
