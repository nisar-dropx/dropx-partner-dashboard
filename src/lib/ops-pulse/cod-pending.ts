...
export type PendingSlip = { id:string; location_id:string|null; deposit_date:string|null; cod_period_from:string|null; cod_period_to:string|null; cod_date:string|null; remittance_code?:string|null; reference_no:string|null; deposited_amount:number|string|null; validated_amount:number|string|null; validation_status:string; ai_status?:string|null; ai_summary?:string|null; ai_result?:Record<string,unknown>|null; proof_checked_at?:string|null; last_updater_name?:string|null; remarks:string|null; validation_remarks:string|null; submitter_name:string|null; created_at:string; attachments:unknown; deposit_slip_attachments:unknown };
export const pendingStatuses = ['Not valid','Validation unavailable','Validation pending','Banker Not Reported','No Cash','Missing slip','Slip missing proof','Rejected','Short','Excess','Duplicate review','Pending verification','Complete'] as const;
...
export function buildCodPendingRows(stations:PendingStation[],slips:PendingSlip[],date:string,now=new Date(),exceptions:CodException[]=[]) {
 ...
 return stations.filter(isCodReportStation).map(station=>{
  ...
  const exception=exceptions.find(e=>e.location_id===station.id);
  const validation=!entries.length?(exception?'Not required':'Validation pending'):entries.some(s=>s.ai_status==='Not valid')?'Not valid':entries.some(s=>s.ai_status==='Validation unavailable')?'Validation unavailable':entries.every(s=>s.ai_status==='Valid'&&hasSlipProof(s))?'Valid':'Validation pending';
  const validationReason=entries.length?[...new Set(entries.map(s=>s.ai_summary).filter(Boolean))].join(' '):exception?.reason||'No daily update has been recorded.';
  const status:PendingStatus=!unique.length?(exception?.kind==='No Cash'?'No Cash':exception?.kind==='Banker Not Reported'?'Banker Not Reported':'Missing slip'):unique.some(s=>!hasSlipProof(s))?'Slip missing proof':unique.some(s=>s.validation_status==='Rejected')?'Rejected':short>0||unique.some(s=>s.validation_status==='Short')?'Short':excess>0||unique.some(s=>s.validation_status==='Excess')?'Excess':duplicates?'Duplicate review':validation==='Not valid'?'Not valid':validation==='Validation unavailable'?'Validation unavailable':validation!=='Valid'?'Validation pending':unique.some(s=>s.validation_status!=='Matched'||nullableMoney(s.deposited_amount)===null)?'Pending verification':'Complete';
  const last=entries[0]?.created_at||null;
  const reportComplete=status==='Complete'||status==='No Cash'||status==='Banker Not Reported';
...
export function codPendingCsv(rows:CodPendingRow[]) {
 ...
 const headers=['Deposit date','Station','Station name','Client','Slip uploaded','Review status','Slip validation','Reason','Email subject','Email confirmation','Last updated by','Deadline IST','Overdue','Late upload','Deposited INR','Portal validated INR','Short INR','Excess INR','Duplicate entries','Remittance codes','COD periods','Last submitted','Remarks'];
 return '\uFEFF'+[headers,...rows.map(r=>[r.date,r.station.station_code,r.station.station_name,r.client,r.slipUploaded?'Yes':'No',r.status,r.validation,r.validationReason,r.exception?.email_subject||'',r.exception?.email_check_status||'',r.entries.map(s=>s.last_updater_name||'').filter(Boolean).join('; ')||r.exception?.updater_name||'',r.date+' 20:30',r.overdue?'Yes':'No',r.late?'Yes':'No',r.amount,r.expected,r.short,r.excess,r.duplicates,r.unique.map(s=>s.remittance_code||s.reference_no||'').join('; '),r.unique.map(s=>(s.cod_period_from||s.cod_date||'')+' to '+(s.cod_period_to||s.cod_period_from||s.cod_date||'')).join('; '),r.last,r.entries.map(s=>s.validation_remarks||s.remarks||'').filter(Boolean).join('; ')])].map(row=>row.map(cell).join(',')).join('\r\n');
}
