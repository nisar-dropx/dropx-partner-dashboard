...
export function buildCodPendingRows(stations:PendingStation[],slips:PendingSlip[],date:string,now=new Date()) {
 ...
 return stations.filter(isCodReportStation).map(station=>{
  ...
  return {station,date,client:codClient(station),status,pending:status!=='Complete',amount:amount===null?null:Math.round(amount*100)/100,expected:expected===null?null:Math.round(expected*100)/100,short:Math.round(short*100)/100,excess:Math.round(excess*100)/100,duplicates,entries,unique,last,deadline:deadline.toISOString(),overdue:status!=='Complete'&&now>=deadline,late:unique.some(s=>Date.parse(s.created_at)>deadline.getTime())};
...
export function filterCodPendingRows(rows:CodPendingRow[],filters:{location?:string;client?:string;status?:string}) {return rows.filter(row=>(!filters.location||row.station.id===filters.location)&&(!filters.client||row.client===filters.client)&&(!filters.status||filters.status==='all'||(filters.status==='pending'?row.pending:row.status===filters.status)));}
export function codPendingCsv(rows:CodPendingRow[]) {
 ...
 const headers=['Deposit date','Station','Station name','Client','Status','Deadline IST','Overdue','Late upload','Deposited INR','Portal validated INR','Short INR','Excess INR','Duplicate entries','Remittance codes','COD periods','Last submitted','Remarks'];
 return '\uFEFF'+[headers,...rows.map(r=>[r.date,r.station.station_code,r.station.station_name,r.client,r.status,r.date+' 20:30',r.overdue?'Yes':'No',r.late?'Yes':'No',r.amount,r.expected,r.short,r.excess,r.duplicates,r.unique.map(s=>s.remittance_code||s.reference_no||'').join('; '),r.unique.map(s=>(s.cod_period_from||s.cod_date||'')+' to '+(s.cod_period_to||s.cod_period_from||s.cod_date||'')).join('; '),r.last,r.entries.map(s=>s.validation_remarks||s.remarks||'').filter(Boolean).join('; ')])].map(row=>row.map(cell).join(',')).join('\r\n');
}
