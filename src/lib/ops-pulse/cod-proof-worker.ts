...
export async function processCodProofChecks(){
 ...
 while(processed<3&&Date.now()-started<190000){
  ...
  const saved=await supabaseAdmin.from('cod_submissions').update({ai_status:result.status,ai_summary:result.reason,ai_result:{model:MODEL,policy_version:2,proof_version:job.proof_version,extracted:result.extracted},proof_checked_at:new Date().toISOString(),proof_check_token:null}).eq('company_id',job.company_id).eq('id',job.id).eq('proof_version',job.proof_version).eq('proof_check_token',job.proof_check_token).select('id');
