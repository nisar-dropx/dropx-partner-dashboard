...
const dispatchStates = new Set(["IN_TRANSIT", "IN_TRANSIT_TO_CUSTOMER", "OUT_FOR_DELIVERY", "DELIVERED", "DELIVERY_ATTEMPTED", "DELIVERY_FAILED", "DELIVERY_REJECTED", "REJECTED"]);
...
export function eddHistoryFacts(history: PackageHistoryEvent[]) {
  ...
  return { firstDispatchAt: first(dispatchStates), firstAttemptAt: first(attemptStates),
    // Empty/malformed history cannot prove that a shipment has never been dispatched.
    historyComplete: history.length > 0 && history.every(e => !!e.time && !Number.isNaN(Date.parse(e.time))) };
}
