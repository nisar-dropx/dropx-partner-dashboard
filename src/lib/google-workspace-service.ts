...
async function provisionWorkspacePerson(job: WorkspaceJobRow) {
  ...
  if (requestedPrimaryEmail && !requestedPrimaryEmail.endsWith(`@${normalizeEmail(setting.primary_domain)}`)) {
  ...
  if (!account && requestedPrimaryEmail) {
    const mapped = await db().from("google_workspace_accounts").select("*")
      .eq("company_id", source.companyId).ilike("primary_email", requestedPrimaryEmail).maybeSingle();
    if (mapped.error) throw new Error(mapped.error.message);
    if (mapped.data?.source_record_id && (mapped.data.source_type !== sourceType || mapped.data.source_record_id !== source.id)) {
      throw new Error("The requested Workspace email is already mapped to another DropX identity.");
    }
    if (mapped.data) {
      account = mapped.data as WorkspaceAccountRow;
      if (account.google_user_id) googleUser = await connection.getUser(account.google_user_id);
    } else {
      googleUser = await connection.getUser(requestedPrimaryEmail);
      if (googleUser) {
  ...
  if (account && !googleUser) googleUser = await connection.getUser(account.google_user_id || account.primary_email);
  if (!googleUser) {
    const localPart = emailLocalPart(policy.email_pattern, source);
    const primaryEmail = account?.primary_email || requestedPrimaryEmail || await availableEmail(connection, setting.primary_domain, localPart, source.employeeCode);
    if (!account) {
  ...
  if (googleUser) {
    const linked = await db().from("google_workspace_accounts").update({
      google_user_id: googleUser.id,
      primary_email: normalizeEmail(googleUser.primaryEmail),
      full_name: googleUser.name?.fullName || source.fullName,
      org_unit_path: googleUser.orgUnitPath || policy.org_unit_path || setting.default_org_unit_path,
    ...
    account = linked.data as WorkspaceAccountRow;
  }
  await db().from("google_workspace_jobs").update({ account_id: account.id, updated_at: new Date().toISOString() }).eq("id", job.id);
