...
export default async function FinanceDashboardPage() {
  ...
  // Read every page so the dashboard does not silently stop at the API row limit.
  for (let offset = 0; ; offset += 1000) {
    if (!supabaseAdmin) { loadError = "Finance data is not configured."; break; }
    ...
    if (!authorization.hasAllLocationAccess) {
      query = query.in("location_id", authorization.locationScopeIds.length ? authorization.locationScopeIds : ["00000000-0000-0000-0000-000000000000"]);
    }
    const result = await query;
    if (result.error) { loadError = result.error.message; break; }
    rows.push(...(result.data ?? []) as PaymentRow[]);
  ...
  return (
    <AppShell active="Finance Dashboard" pageCode="payments">
      <PageHead
        ...
        subtitle="Review approvals, payment readiness and completed payments across your permitted locations."
