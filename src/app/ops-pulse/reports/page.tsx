...
export default async function OpsReportsPage() {
  ...
  return <AppShell active="Reports" pageCode="ops_reports"><div className="ops-command-center ops-reports-workspace">
    ...
    {hasPermission(authorization, "performance_review", "access") ? <ReviewReportBuilder stations={locations.map((row) => ({ code: row.station_code, name: row.station_name || row.city || row.station_code, cluster: row.cluster || "Unassigned" }))}/> : null}
