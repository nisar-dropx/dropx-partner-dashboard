...
export default async function CodPendingPage({searchParams={}}:{searchParams?:Params}){
 ...
 return <>
 <PageHead eyebrow="Ops Pulse" title="Daily COD Pending" subtitle="Check which stations have uploaded their daily COD slip and which have not. Deadline: 8:30 PM IST. Open a station for proof and review details."/>
 ...
 <section className="panel"><div className="panel-body" style={{display:'flex',flexWrap:'wrap',gap:32}}>{[['Stations',scoped.length],['Slip uploaded',scoped.filter(r=>r.slipUploaded).length],['Not uploaded',scoped.filter(r=>!r.slipUploaded).length],['Needs attention',scoped.filter(r=>r.pending).length],['Complete',scoped.filter(r=>!r.pending).length],['Known shortage',amount(scoped.reduce((n,r)=>n+r.short,0))]].map(([label,value])=><div key={label}><div className="subtle">{label}</div><strong style={{fontSize:24}}>{value}</strong></div>)}</div></section>
 ...
 <div className="table-wrap"><table><thead><tr><th>Station / details</th><th>Client</th><th>Slip uploaded</th><th>Review status</th><th>Deposited</th><th>Short</th><th>Last upload</th></tr></thead><tbody>
