...
export default async function CodPendingPage({searchParams={}}:{searchParams?:Params}){
 ...
 const date=searchParams.date||todayKolkata(),status=searchParams.status||'pending';
 ...
 return <>
 <PageHead eyebrow="Ops Pulse" title="Daily COD Pending" subtitle="Every station must upload its deposit slip by 8:30 PM IST. Open a station to inspect proof, shortages and remarks."/>
 ...
 <label>Status<select className="field" name="status" defaultValue={status}><option value="pending">Pending / needs attention</option><option value="all">All statuses</option>{pendingStatuses.map(s=><option key={s}>{s}</option>)}</select></label>
 <div className="form-actions span-4"><button className="button primary" type="submit">Show daily report</button>{!error?<a className="button secondary" href={'/api/ops-pulse/cod/pending/export?'+query}>Download CSV</a>:null}{hasPermission(auth,'ops_notification_settings','access')?<Link className="button secondary" href="/settings/notifications#cod-pending">Email schedule</Link>:null}</div>
 </form></div></section>
 {error?<section className="panel message-panel error"><div className="panel-body" role="alert">{error}</div></section>:<>
 <section className="panel"><div className="panel-body" style={{display:'flex',flexWrap:'wrap',gap:32}}>{[['Stations',scoped.length],['Missing slips',scoped.filter(r=>r.status==='Missing slip'||r.status==='Slip missing proof').length],['Needs attention',scoped.filter(r=>r.pending).length],['Complete',scoped.filter(r=>!r.pending).length],['Known shortage',amount(scoped.reduce((n,r)=>n+r.short,0))]].map(([label,value])=><div key={label}><div className="subtle">{label}</div><strong style={{fontSize:24}}>{value}</strong></div>)}</div></section>
 ...
 <div className="table-wrap"><table><thead><tr><th>Station / details</th><th>Client</th><th>Slip status</th><th>Deposited</th><th>Short</th><th>Last upload</th></tr></thead><tbody>
