...
type SearchParams = { from?: string; to?: string; preset?: string; stations?: string; cluster?: string; sort?: string; dir?: string };
...
export default async function LowProductivityAssociatesPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const sort = ["associate", "station", "days", "delivery", "returns", "total", "productivity", "cps", "status"].includes(String(searchParams?.sort)) ? String(searchParams?.sort) : "productivity";
  ...
  const threshold = Math.min(...rules.rows.map((rule) => rule.targetSpr).filter((value): value is number => value != null), 40);
  const rows = [...people.values()].map((row) => { const total = row.delivery + row.returns; const productivity = row.days ? total / row.days : 0; return { ...row, total, productivity, cps: total ? row.paid / total : null, target: ruleByStation.get(row.station)?.targetSpr ?? threshold }; }).filter((row) => row.productivity < row.target).sort((left, right) => { const value = (row: typeof left) => sort === "associate" ? row.name : sort === "station" ? row.station : sort === "days" ? row.days : sort === "delivery" ? row.delivery : sort === "returns" ? row.returns : sort === "total" ? row.total : sort === "cps" ? (row.cps ?? -1) : sort === "status" ? "Below target" : row.productivity; const a = value(left), b = value(right); const compared = typeof a === "string" ? a.localeCompare(String(b)) : Number(a) - Number(b); return (dir === "asc" ? compared : -compared) || left.name.localeCompare(right.name); });
  ...
  const sortHref = (column: string) => { const params = new URLSearchParams({ from: start, to: end, preset, sort: column, dir: sort === column && dir === "asc" ? "desc" : "asc" }); if (cluster) params.set("cluster", cluster); if (searchParams?.stations) params.set("stations", searchParams.stations); return `/ops-pulse/capacity/associates/low-productivity?${params.toString()}`; };
  const sortMark = (column: string) => sort === column ? (dir === "asc" ? "↑" : "↓") : "↕";
