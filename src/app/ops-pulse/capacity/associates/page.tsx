...
export default async function SprAssociatesPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  if (selectedCodes.length !== locations.length) exportParams.set("stations", selectedCodes.length ? selectedCodes.join(",") : "_none");
  const exportHref = `/api/ops-pulse/capacity/associates/export?${exportParams.toString()}`;
  ...
  return <AppShell active="Capacity" pageCode="capacity_associates"><div className="ops-command-center capacity-workspace">
    ...
    {activeView === "productivity" ? <>
      ...
      <section className="panel"><div className="panel-head"><div><h2>Associate productivity</h2><p className="subtle">SPR = Amazon delivery count ÷ active days. Select an associate for the daily trend.</p></div><div className="capacity-table-actions"><a className="button secondary compact" href={exportHref}>Download Excel</a><div className="capacity-table-pager"><span>{associates.length ? `${(currentPage - 1) * pageSize + 1}–${Math.min(currentPage * pageSize, associates.length)} of ${associates.length}` : "0 associates"}</span>{currentPage > 1 ? <a href={pageHref(currentPage - 1)}>←</a> : <i>←</i>}<b>{currentPage}/{pageCount}</b>{currentPage < pageCount ? <a href={pageHref(currentPage + 1)}>→</a> : <i>→</i>}</div></div></div><div className="table-wrap"><table className="capacity-daily-table"><thead><tr><th><a href={sortHref("name")}>Associate <small>{sortMark("name")}</small></a></th><th><a href={sortHref("station")}>Station <small>{sortMark("station")}</small></a></th><th><a href={sortHref("days")}>Active days <small>{sortMark("days")}</small></a></th><th><a href={sortHref("delivered")}>Amazon deliveries <small>{sortMark("delivered")}</small></a></th><th><a href={sortHref("average")}>Average SPR <small>{sortMark("average")}</small></a></th><th><a href={sortHref("peak")}>Peak <small>{sortMark("peak")}</small></a></th><th><a href={sortHref("highDays")}>High days <small>{sortMark("highDays")}</small></a></th><th><a href={sortHref("level")}>SPR position <small>{sortMark("level")}</small></a></th></tr></thead><tbody>
