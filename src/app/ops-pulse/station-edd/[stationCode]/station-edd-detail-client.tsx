...
import { applyEddLookup, eddCurrentState, eddSourceMeaning } from "@/lib/ops-pulse/edd-verification";
...
import { EddAttemptHolds } from "../edd-attempt-holds";
...
export function StationEddDetailClient({ stationCode, initialQuery = {} }: { stationCode: string; initialQuery?: EddQuery }) {
  ...
  return <div className={s.workspace}>
    ...
    {loading ? <div className={s.empty}>Loading station EDDs…</div> : !payload ? <div className={s.empty}>No observed records yet. Use Source tools to refresh this station. Missing data is not a zero count.</div> : <>
      ...
      <p className={s.tableHelp}>Recorded data reloads every minute. Source collection targets 15 minutes. Pending needs current per-package evidence; incomplete histories remain separate. {summary.todayUnverified > 0 ? "Pending coverage is incomplete — zero confirmed pending does not mean the station is cleared." : ""}</p>
      <section className={s.metrics} aria-label="Today's station EDD position">{([
        ...
        ["hfr", "HFR · prior attempt", summary.todayHfr, "One verified prior-day attempt", "purple"],
        ["hcr", "HCR · prior attempts", summary.todayHcr, "Two or more distinct attempts", "purple"],
        ["unverified", "Needs history check", summary.todayUnverified, "Excluded from confirmed pending", "neutral"]
      ] as const).map(([key, label, value, hint, tone]) => <button key={key} aria-pressed={controls.view === "tids" && controls.position === key && controls.day === "today"} className={[s.metric, s[tone], controls.view === "tids" && controls.position === key && controls.day === "today" ? s.selectedMetric : ""].join(" ")} onClick={() => select(key, "today")}><span>{label}</span><strong>{key === "atStation" && summary.todayUnverified > 0 ? `${n(value)} confirmed` : n(value)}</strong><small>{hint}</small></button>)}</section>
      <div className={s.inlineStats}><span>Observed INDUCTED / RECEIVED today <strong>{n(summary.todayObservedAtStation)}</strong></span><span className={s.muted}>Includes returned and unchecked parcels — not the confirmed pending count.</span></div>
      <div className={s.coverageCompact}><ShieldCheck size={18}/><div><strong>{summary.todayUnverified ? n(summary.todayUnverified) + " due-today TIDs need a history check" : "No due-today TIDs are waiting for pending classification"}</strong><span>{n(summary.todayTotal)} known EDDs today · {n(todayAssociates.reduce((sum, a) => sum + a.sent, 0))} sent. {summary.missingDate ? n(summary.missingDate) + " records have an unconfirmed EDD date." : "No missing EDD dates in the retained records."}</span><details><summary>What does this mean?</summary><p>Pending needs complete, current history proving no dispatch or attempt. On-road and delivered outcomes can be known from source scans without fetching every historical event. {n(summary.historyVerified)} of today’s TIDs have complete history stored; this is not an overall completion percentage. One-attempt HFR, two-attempt HCR and rejections are distinguished in the return panel below. Counts reflect observations and can change after refresh.</p></details></div></div>
      <EddAttemptHolds packages={packages} stationCode={stationCode} now={clock} onOpen={setOpenTid}/>
      <section className={s.panel}>
        ...
        <div role="tabpanel" id={"panel-" + controls.view} aria-labelledby={"tab-" + controls.view}>
          ...
          {controls.view === "associates" ? <>
            ...
            <div className={s.tableWrap}><table className={s.table} aria-label="Source status results"><thead><tr>{STATUS_SORTS.map(([key, label]) => <SortHeader key={key} column={key} label={key === "count" ? periodLabel : label} sort={controls.statusSort} direction={controls.statusDirection} onSort={sort} numeric={key !== "state"}/>) }<th>Meaning</th></tr></thead><tbody>{statuses.slice(start, start + size).map(row => <tr key={row.state}><td>{row.state}</td><td className={s.numeric}><button className={s.numberLink} aria-label={row.state + " selected period: " + row.count + " TIDs"} onClick={() => select("all", controls.day, "", row.state)}>{n(row.count)}</button></td><td className={s.numeric}><button className={s.numberLink} aria-label={row.state + " all dates: " + row.total + " TIDs"} onClick={() => select("all", "all", "", row.state)}>{n(row.total)}</button></td><td>{eddSourceMeaning(row.state)}</td></tr>)}</tbody></table></div>
    ...
    <TrackingDetailModal trackingId={openTid} stationHint={stationCode} onLookupComplete={result => {
      if (result.trackingId !== openTid) return;
      ...
      setClock(Date.now()); setMessage(result.stationCode === stationCode ? "Live tracking result applied to the table and station counts." : `Source route is ${result.stationCode}. Evidence saved; station attribution needs reconciliation.`);
