...
import "./review-targets.css";
import { PerformanceReviewTargetMaster } from "@/components/performance-review-target-master";
import { PerformanceReviewOversightMaster } from "@/components/performance-review-oversight-master";
import { PerformanceStationTargetMaster } from "@/components/performance-station-target-master";
import { loadStationReviewTargets } from "@/lib/ops-pulse/station-review-targets-data";
...
import { loadOversightDesignationOptions, loadReviewOversightRoles } from "@/lib/ops-pulse/review-oversight-roles";
import { addPerformanceMetric, addPerformanceReviewOversightRole, removePerformanceMetric, removePerformanceReviewOversightRole, updatePerformanceReviewCadence, updatePerformanceReviewOversightRole, updatePerformanceStationOpeningWindow, updatePerformanceTarget } from "./actions";

export const dynamic = "force-dynamic";
type SearchParams = { targets_saved?: string; view?: string; saved?: string; opening_saved?: string; added?: string; deleted?: string; oversight_saved?: string; oversight_deleted?: string; error?: string };
...
export default async function PerformanceTargetMaster({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const view = searchParams?.view === "daily" ? "daily" : searchParams?.view === "reviews" ? "reviews" : searchParams?.view === "sls" ? "sls" : "targets";
  ...
  const stationTargets = view === "targets" ? await loadStationReviewTargets(companyId, permittedLocations.map(row=>row.id)) : {rows:[],error:null};
  const oversightRoles = view === "reviews" ? await loadReviewOversightRoles(companyId) : {rows:[],error:null};
  const oversightDesignations = view === "reviews" ? await loadOversightDesignationOptions(companyId) : {rows:[],error:null};
  const usedOversightMatchTexts = new Set(oversightRoles.rows.map((row) => row.matchText));
  const oversightDesignationOptions = oversightDesignations.rows.filter((designation) => !usedOversightMatchTexts.has(designation.name.trim().toUpperCase().replace(/[^A-Z0-9]+/g, " ").replace(/\s+/g, " ").trim()));
  ...
  return <AppShell active="Performance Master" pageCode="performance_master"><div className="ops-command-center">
    ...
    <nav className="performance-tabs"><Link className={view === "targets" ? "active" : ""} href="/master/performance-targets?view=targets">Review targets</Link><Link className={view === "sls" ? "active" : ""} href="/master/performance-targets?view=sls">Weekly SLS</Link><Link className={view === "daily" ? "active" : ""} href="/master/performance-targets?view=daily">Daily performance</Link><Link className={view === "reviews" ? "active" : ""} href="/master/performance-targets?view=reviews">Review cadence</Link></nav>
    {searchParams?.targets_saved ? <section className="message-panel success">Targets saved. Existing review entries are unchanged.</section> : null}
