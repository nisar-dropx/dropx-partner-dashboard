...
import { addPerformanceMetric, removePerformanceMetric, updatePerformanceReviewCadence, updatePerformanceTarget } from "./actions";
...
export default async function PerformanceTargetMaster({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const [sourceResult, cadenceResult] = supabaseAdmin
    ? await Promise.all([
  ...
  return <AppShell active="Performance Master" pageCode="performance_master"><div className="ops-command-center">
    ...
    {view === "reviews" ? <section className="panel"><div className="panel-head"><div><h2>Review cadence</h2><p className="subtle">Set when daily operations and weekly sales reviews are expected. The reporting hierarchy controls who reviews each station.</p></div></div><form action={updatePerformanceReviewCadence} className="performance-review-cadence"><label>Daily review time<input type="time" name="daily_review_time" defaultValue={String(cadenceResult.data?.daily_review_time ?? "10:00").slice(0,5)}/></label><label>Weekly sales day<select name="weekly_review_weekday" defaultValue={Number(cadenceResult.data?.weekly_review_weekday ?? 4)}><option value="1">Monday</option><option value="2">Tuesday</option><option value="3">Wednesday</option><option value="4">Thursday</option><option value="5">Friday</option><option value="6">Saturday</option><option value="0">Sunday</option></select></label><label>Weekly review time<input type="time" name="weekly_review_time" defaultValue={String(cadenceResult.data?.weekly_review_time ?? "16:00").slice(0,5)}/></label><label>Stuck after<input type="number" name="stale_after_hours" min="1" max="168" defaultValue={Number(cadenceResult.data?.stale_after_hours ?? 24)}/><small>Hours at the same review level</small></label><SubmitButton disabled={!permission.canEdit}>Save cadence</SubmitButton></form></section> : <><section className="performance-summary-grid"><article><span>Metrics</span><strong>{rows.length}</strong><small>{view === "sls" ? "Weekly SLS" : "Daily performance"}</small></article><article><span>Total weight</span><strong>{totalWeight}%</strong><small>{view === "sls" ? "Must total 100%" : "Informational targets"}</small></article><article><span>Mapped fields</span><strong>{rows.filter((row) => row.sourceIndex != null).length}</strong><small>Available in report</small></article><article><span>Unmapped</span><strong>{rows.filter((row) => row.sourceIndex == null).length}</strong><small>Awaiting source field</small></article></section>
