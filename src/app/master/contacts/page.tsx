...
type Contact = { id: string; contact_no: string | null; email: string | null; bank_account_no: string | null; ifsc: string | null; upi_id: string | null; account_holder_name: string; verified_at: string | null };
...
export default async function ContactsPage({ searchParams }: { searchParams?: { edit?: string } }) {
  ...
  const result = await supabaseAdmin.from("payment_contacts").select("id, contact_no, email, bank_account_no, ifsc, upi_id, account_holder_name, verified_at").eq("company_id", companyId).order("account_holder_name");
