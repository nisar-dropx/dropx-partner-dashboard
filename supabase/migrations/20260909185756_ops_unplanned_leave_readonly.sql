...
roster as (
 ...
 coalesce(g.location_id,w.location_id) current_location_id,coalesce(g.designation_name,w.designation_name) role_name,g.department_name,
...
evidence as (
 ...
 exists(select 1 from public.hr_wfh_requests f where f.company_id=r.company_id and f.status='approved' and f.profile_id=r.worker_id and f.profile_type=r.worker_type and r.attendance_date between f.start_date and f.end_date) approved_wfh,
