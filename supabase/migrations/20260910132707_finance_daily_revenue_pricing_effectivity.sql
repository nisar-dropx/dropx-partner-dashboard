-- Expose C-returns at daily grain so Finance can reproduce the Amazon MG
-- billing volume from the invoice calculation: deliveries + C-returns.
    ...
    coalesce((
      ...
      audit as (
        select
          source_id,
          case
            when count(*) filter (where smd is null or smd2 is null) = 0
              then sum(smd + smd2)
          end as smd,
          case
            when count(*) filter (where ihs is null) = 0
              then sum(ihs)
          end as ihs
        from metrics
        group by source_id
      ...
      from (
        select
          ...
          c.work_date,
          case
            when count(*) filter (where c.total_delivery is null) = 0
              then sum(c.total_delivery)::text
          end as deliveries,
          case
            when count(*) filter (where c.amazon_delivery is null) = 0
              then sum(c.amazon_delivery)::text
          end as mg_deliveries,
          case
            when count(*) filter (where c.swa_delivery is null) = 0
              then sum(c.swa_delivery)::text
          end as swa,
          case
            when count(*) filter (where c.c_return is null) = 0
              then sum(c.c_return)::text
          end as returns,
          case
            when count(*) filter (where c.mfn is null) = 0
              then sum(c.mfn)::text
          end as mfn,
          case
            when count(*) filter (where a.smd is null or a.smd > c.total_delivery) = 0
              then sum(a.smd)::text
          end as smd,
          case
            when count(*) filter (where a.ihs is null) = 0
              then sum(a.ihs)::text
          end as ihs,
