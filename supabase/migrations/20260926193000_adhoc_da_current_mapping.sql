-- Resolve the DA identity from the current Provider Mapping. The selected work
-- date is only the payroll-recovery effective date; it must never decide which
-- roster identity or Workforce mapping is valid.
...
begin
  ...
  if tg_op='INSERT' or old.adhoc_workforce_id is null then
    ...
    if new.adhoc_work_date is null then raise exception 'Select the payroll deduction work date'; end if;
    ...
    if new.adhoc_shipment_id is not null then
      -- The shipment row supplies only the latest imported DA identity. Its
      -- report date is intentionally independent from the recovery work date.
      ...
      select array_agg(distinct w.id) into workers
        ...
        where m.company_id=new.company_id and m.station_id=new.location_id and btrim(m.provider_member_id)=btrim(shipment.provider_employee_id)
      ...
      if coalesce(cardinality(workers),0)<>1 then raise exception 'This DA ID needs one active Workforce mapping for this station. Correct Provider Mapping before submitting payment'; end if;
      new.adhoc_workforce_id:=workers[1];
      new.adhoc_provider_employee_id:=btrim(shipment.provider_employee_id);
      new.adhoc_da_name:=coalesce(nullif(btrim(shipment.provider_employee_name),''),btrim(shipment.provider_employee_id));
