-- Blank person cluster (stations.cluster_name often stores a cluster-manager name) and expose N+1 manager.
-- Shifts stay roster-only. Read-only projection. Service-role only.
...
roster as (
 select distinct on (w.worker_type,w.worker_id) w.company_id,w.worker_type,w.worker_id,w.biometric_id,w.date_of_join,w.email,w.mobile,
 w.people_lifecycle_status,w.last_working_date,p.attendance_date,g.person_id,g.assignment_id,g.manager_assignment_id,g.manager_person_ids,
 coalesce(nullif(trim(g.display_name),''),w.full_name) full_name,
 coalesce(nullif(trim(g.worker_code),''),w.worker_code) worker_code,
 coalesce(g.location_id,w.location_id) people_location_id,
 g.designation_name role_name,g.department_name,
 (select bg.display_name from base_graph bg where bg.person_id=g.manager_person_ids[1] limit 1) manager_name,
 (select bg.display_name from base_graph bg where bg.person_id=g.manager_person_ids[2] limit 1) next_manager_name,
...
evidence as (
 select r.*,s.station_code,s.station_name,s.station_email,null::text as cluster,s.region,
 ...
 and (cal.location_id is null or cal.location_id=r.people_location_id)
 ...
 from roster r left join public.stations s on s.company_id=r.company_id and s.id=r.people_location_id
),
candidates as (
 select person_id,assignment_id,worker_type,worker_id,worker_code,full_name,mobile,role_name,department_name,manager_name,next_manager_name,manager_assignment_id,manager_person_ids,people_location_id location_id,station_code,station_name,cluster,region,attendance_date,shift_code,shift_start,shift_end
 ...
 and (not v_location_account or people_location_id=any(p_location_ids))
