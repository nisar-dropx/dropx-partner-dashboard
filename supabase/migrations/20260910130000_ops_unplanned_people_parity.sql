-- Align Ops Unplanned Leaves identity with live People (assignment org + reporting manager).
-- Shifts stay roster-only. Location/name/role no longer prefer roster or employment free-text over People.
-- Read-only projection. Service-role only.
...
candidates as (
 select person_id,assignment_id,worker_type,worker_id,worker_code,full_name,mobile,role_name,department_name,manager_name,manager_assignment_id,manager_person_ids,people_location_id location_id,station_code,station_name,cluster,region,attendance_date,shift_code,shift_start,shift_end
