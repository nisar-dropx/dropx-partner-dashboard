...
export function ConnectPerformance({ account }: { account: AppAccount }) {
  ...
  return <section className="dx-performance dx-performance-product">
    <header className="dx-page-intro dx-performance-intro"><div><small>Performance</small><h1>My performance</h1><p>Operational scorecards, monthly CPS and individual reviews.</p></div>{section === "scorecards" && weekPeriods.length ? <label><span className="sr-only">Scorecard week</span><select value={weekKey ?? ""} onChange={(event) => setWeekKey(Number(event.target.value))}>{weekPeriods.map((item) => <option key={item.key} value={item.key}>Week {item.week}{weekRangeLabel(item.startDate, item.endDate) ? ` · ${weekRangeLabel(item.startDate, item.endDate)}` : ""}</option>)}</select><ChevronDown /></label> : section === "cps" && operational?.availableCpsMonths.length ? <label><span className="sr-only">CPS month</span><select value={cpsMonth} onChange={(event) => setCpsMonth(event.target.value)}>{operational.availableCpsMonths.map((item) => <option key={item} value={item}>{monthLabel(item)}{item === currentMonthIndia() ? " · MTD" : ""}</option>)}</select><ChevronDown /></label> : null}</header>
    <nav className="dx-performance-sections" aria-label="Performance sections"><button className={section === "scorecards" ? "active" : ""} onClick={() => setSection("scorecards")}><BarChart3 /> Scorecard</button><button className={section === "cps" ? "active" : ""} onClick={() => setSection("cps")}><CircleGauge /> CPS</button><button className={section === "reviews" ? "active" : ""} onClick={() => setSection("reviews")}><Award /> Reviews{data.managerReviews.length ? <b>{data.managerReviews.length}</b> : null}</button></nav>
