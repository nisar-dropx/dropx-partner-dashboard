...
import { Award, CalendarDays, Check, ChevronDown, CircleGauge, Sparkles, Target, TrendingUp } from "lucide-react";
...
function Status({ value }: { value: string }) {
  const tone = ["finalised", "acknowledged", "completed", "approved", "applied"].includes(value)
    ? "good"
    : ["at_risk", "blocked", "rejected", "cancelled"].includes(value)
      ? "danger"
      : "pending";
...
export function ConnectPerformance({ account }: { account: AppAccount }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/connect/performance?${query}`, { cache: "no-store" })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(payload.error || "Unable to load performance reviews.");
        return payload as Payload;
      })
      .then((payload) => {
        setData(payload);
        setActiveReviewId((current) => payload.reviews.some((item) => item.id === current) ? current : payload.reviews[0]?.id ?? "");
      })
      .catch((reason) => setError(reason instanceof Error ? reason.message : "Unable to load performance reviews."))
  ...
  async function submit(body: Record<string, unknown>) {
    ...
    setNotice("");
    try {
      const response = await fetch("/api/connect/performance", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...body, accountId: account.id, profileType: account.profileType })
      });
      const payload = await response.json();
      ...
      setError(reason instanceof Error ? reason.message : "Unable to save performance details.");
    } finally {
      setSaving(false);
    }
  }

  function submitSelfReview(event: FormEvent<HTMLFormElement>) {
  ...
  function updateGoal(event: FormEvent<HTMLFormElement>, goal: Goal) {
    event.preventDefault();
    if (!activeReview) return;
    const form = new FormData(event.currentTarget);
    void submit({
  ...
  if (loading) return <div className="dx-loader fullscreen"><span /><small>Loading performance...</small></div>;
  if (error && !data) return <div className="dx-alert error">{error}<button onClick={() => setRefresh((value) => value + 1)}>Retry</button></div>;
  if (!data?.configured) return <section className="dx-performance"><header className="dx-page-intro"><small>Growth</small><h1>Performance</h1><p>Your People profile needs to be linked before reviews can begin.</p></header><div className="dx-performance-empty"><Target /><strong>Performance profile not configured</strong><span>Ask HR to complete your People engagement and reporting-manager mapping.</span></div></section>;
  if (!data.reviews.length) return <section className="dx-performance"><header className="dx-page-intro"><small>Growth</small><h1>Performance</h1><p>Goals, feedback and outcomes in one place.</p></header><div className="dx-performance-empty"><Sparkles /><strong>No active review yet</strong><span>Your next review will appear here when HR launches a performance cycle.</span></div></section>;

  return <section className="dx-performance">
    <header className="dx-page-intro dx-performance-intro"><div><small>Performance & growth</small><h1>My progress</h1><p>Goals, feedback and review outcomes—clear and private.</p></div><label><span className="sr-only">Performance cycle</span><select value={activeReview?.id ?? ""} onChange={(event) => setActiveReviewId(event.target.value)}>{data.reviews.map((review) => <option key={review.id} value={review.id}>{cycles.get(review.cycle_id)?.name ?? "Performance review"}</option>)}</select><ChevronDown /></label></header>
    {error ? <div className="dx-alert error">{error}</div> : null}
    {notice ? <div className="dx-alert success">{notice}</div> : null}

    <section className="dx-performance-hero">
      <div><span><Award /> {activeReview?.designation_name || "People team"}</span><h2>{activeCycle?.name ?? "Performance review"}</h2><p>{date(activeCycle?.period_start)} – {date(activeCycle?.period_end)}</p></div>
      <Status value={activeReview?.status ?? "assigned"} />
      <dl><div><dt><Target /> Goals</dt><dd>{completedGoals}/{goals.length}</dd></div><div><dt><CircleGauge /> Progress</dt><dd>{averageProgress}%</dd></div><div><dt><TrendingUp /> Rating</dt><dd>{activeReview?.final_rating ?? activeReview?.manager_rating ?? activeReview?.self_rating ?? "—"}<small>/{activeCycle?.rating_scale ?? 5}</small></dd></div></dl>
    </section>

    {activeReview?.status === "assigned" ? <form className="dx-performance-card dx-performance-self" onSubmit={submitSelfReview}>
      <header><div><small>Your reflection</small><h2>Self-review</h2></div><span><CalendarDays /> Due {date(activeCycle?.self_review_due)}</span></header>
      <label>How would you rate this period?<input max={activeCycle?.rating_scale ?? 5} min="0" name="selfRating" required step="0.1" type="number" /></label>
      <label>Contribution and development focus<textarea minLength={10} name="comments" placeholder="What went well? Where would support help?" required /></label>
      <button disabled={saving}>{saving ? "Submitting..." : "Submit to manager"}</button>
    </form> : null}
