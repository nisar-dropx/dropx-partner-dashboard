...
type Operational = {
  scopeLabel: string; stationCount: number; availableWeeks: number[]; availableWeekPeriods?: Array<{ key: number; week: number; year: number; startDate: string; endDate: string }>; selectedWeek: number | null; selectedYear: number | null; selectedWeekKey?: number | null; selectedWeekStart?: string | null; selectedWeekEnd?: string | null; averageSls: number | null; averageAttainment: number | null; availableCpsMonths: string[]; selectedCpsMonth: string; cpsPeriodState: "mtd" | "closed"; cpsPeriodLabel: string; cpsLatestDate: string | null; averageCps: number | null; cpsOnTarget: number; cpsMeasured: number; standingCounts: Record<string, number>;
  stations: Array<{ id: string; code: string; name: string; region: string | null; model: string | null; unitType: "station" | "store" | "hub" | "location"; sls: { score: number; standing: string; achievedWeight: number; availableWeight: number; attainment: number; metrics: Metric[] } | null; cps: { date: string; value: number; target: number | null; gap: number | null; onTarget: boolean | null } | null }>;
};
...
function unitPerformance(value: Operational["stations"][number]["unitType"]) { return `${value.charAt(0).toUpperCase()}${value.slice(1)} performance`; }
function currentMonthIndia() { return new Intl.DateTimeFormat("en-CA", { year: "numeric", month: "2-digit", timeZone: "Asia/Kolkata" }).format(new Date()); }
function monthLabel(value: string) { const parsed = new Date(`${value}-15T12:00:00+05:30`); return Number.isNaN(parsed.getTime()) ? value : new Intl.DateTimeFormat("en-IN", { month: "long", year: "numeric" }).format(parsed); }
function weekRangeLabel(start?: string | null, end?: string | null) {
  ...
  const day = (value: Date) => new Intl.DateTimeFormat("en-IN", { day: "numeric", timeZone: "UTC" }).format(value);
  const month = (value: Date) => new Intl.DateTimeFormat("en-IN", { month: "short", timeZone: "UTC" }).format(value);
  const year = new Intl.DateTimeFormat("en-IN", { year: "numeric", timeZone: "UTC" }).format(to);
  return from.getUTCMonth() === to.getUTCMonth()
    ? `${day(from)}–${day(to)} ${month(to)} ${year}`
    : `${day(from)} ${month(from)}–${day(to)} ${month(to)} ${year}`;
}

function Status({ value }: { value: string }) {
  const normalized = value.toLowerCase();
  const tone = ["finalised", "acknowledged", "completed", "approved", "applied", "fantastic", "great", "on target"].includes(normalized) ? "good" : ["at_risk", "blocked", "rejected", "cancelled", "poor", "missed"].includes(normalized) ? "danger" : "pending";
...
export function ConnectPerformance({ account }: { account: AppAccount }) {
  ...
  const [cpsMonth, setCpsMonth] = useState(currentMonthIndia);
  ...
  async function submit(body: Record<string, unknown>) {
    ...
    try {
      ...
      const payload = await readJsonResponse<{ notice?: string }>(response, "Unable to save performance details. Please try again.");
      setNotice(payload.notice || "Saved."); setRefresh((value) => value + 1);
    } catch (reason) { setError(userFacingError(reason, "Unable to save performance details. Please try again.")); } finally { setSaving(false); }
  }
  ...
  const weekPeriods = operational?.availableWeekPeriods ?? (operational?.availableWeeks ?? []).map((item) => ({ key: Number(operational?.selectedYear ?? 0) * 100 + item, week: item, year: Number(operational?.selectedYear ?? 0), startDate: "", endDate: "" }));
  const selectedWeekRange = weekRangeLabel(operational?.selectedWeekStart, operational?.selectedWeekEnd);
  const scorecardStations = operational?.stations.filter((station) => station.sls) ?? [];
  const cpsStations = operational?.stations.filter((station) => station.cps) ?? [];
  return <section className="dx-performance dx-performance-product">
    <header className="dx-page-intro dx-performance-intro"><div><small>Performance</small><h1>My performance</h1><p>{operational ? "Operational scorecards, monthly CPS and individual reviews." : "Your goals, feedback and individual performance reviews."}</p></div>{visibleSection === "scorecards" && weekPeriods.length ? <label><span className="sr-only">Scorecard week</span><select value={weekKey ?? ""} onChange={(event) => setWeekKey(Number(event.target.value))}>{weekPeriods.map((item) => <option key={item.key} value={item.key}>Week {item.week}{weekRangeLabel(item.startDate, item.endDate) ? ` · ${weekRangeLabel(item.startDate, item.endDate)}` : ""}</option>)}</select><ChevronDown /></label> : visibleSection === "cps" && operational?.availableCpsMonths.length ? <label><span className="sr-only">CPS month</span><select value={cpsMonth} onChange={(event) => setCpsMonth(event.target.value)}>{operational.availableCpsMonths.map((item) => <option key={item} value={item}>{monthLabel(item)}{item === currentMonthIndia() ? " · MTD" : ""}</option>)}</select><ChevronDown /></label> : null}</header>
    {operational ? <nav className="dx-performance-sections" aria-label="Performance sections"><button className={visibleSection === "scorecards" ? "active" : ""} onClick={() => setSection("scorecards")}><BarChart3 /> Scorecard</button><button className={visibleSection === "cps" ? "active" : ""} onClick={() => setSection("cps")}><CircleGauge /> CPS</button><button className={visibleSection === "reviews" ? "active" : ""} onClick={() => setSection("reviews")}><Award /> Individual{data.managerReviews.length ? <b>{data.managerReviews.length}</b> : null}</button></nav> : null}
    {error ? <div className="dx-alert error">{error}</div> : null}{notice ? <div className="dx-alert success">{notice}</div> : null}

    {visibleSection === "scorecards" ? <>
      <section className="dx-performance-portfolio-hero"><div><span><MapPinned /> {operational?.scopeLabel ?? "People scope"}</span><strong>{operational?.selectedWeek ? `Scorecard · Week ${operational.selectedWeek}` : "Operational scorecard"}</strong><small>{selectedWeekRange || operational?.selectedYear || "Latest uploaded week"}</small></div><dl><div><dt>Average score</dt><dd>{percent(operational?.averageSls)}</dd></div><div><dt>Targets met</dt><dd>{operational?.averageAttainment == null ? "—" : `${operational.averageAttainment}%`}</dd></div><div><dt>Scored locations</dt><dd>{scorecardStations.length || "—"}</dd></div></dl></section>
      {scorecardStations.length ? <section className="dx-performance-station-list"><header><div><small>My portfolio</small><h2>{scorecardStations.length} location scorecard{scorecardStations.length === 1 ? "" : "s"}</h2></div><span>Open for details</span></header>{scorecardStations.map((station) => <details key={station.id} className="dx-performance-station-card" open={scorecardStations.length === 1}><summary><div><i>{station.code}</i><span><strong>{station.name}</strong><small>{unitPerformance(station.unitType)} · {[station.model, station.region].filter(Boolean).join(" · ") || "People mapped"}</small></span></div><div><Status value={station.sls!.standing} /><b>{percent(station.sls!.score)}</b></div><ChevronDown /></summary><div className="dx-performance-station-detail"><div className="dx-performance-station-summary compact"><span><small>Score</small><strong>{percent(station.sls!.score)}</strong></span><span><small>Targets met</small><strong>{station.sls!.attainment}%</strong></span></div>{station.sls!.metrics.length ? <div className="dx-performance-metric-grid">{station.sls!.metrics.map((metric) => <article className={metric.status} key={metric.key}><span>{metric.label}</span><strong>{metricValue(metric)}</strong><small>{metric.status === "missed" ? "Target missed · " : "Target "}{metricTarget(metric)}{metric.weight ? ` · ${metric.weight}%` : ""}</small></article>)}</div> : <div className="dx-performance-empty compact"><Target /><strong>Detailed metrics pending</strong><span>This uploaded scorecard has no active mapped metrics.</span></div>}</div></details>)}</section> : <div className="dx-performance-empty"><BarChart3 /><strong>No scorecard is available for your mapped locations</strong><span>Your weekly scorecard appears after the upload is completed.</span></div>}
    </> : visibleSection === "cps" ? <>
      <section className="dx-performance-portfolio-hero cps"><div><span><CircleGauge /> {operational?.scopeLabel ?? "People scope"}</span><strong>CPS · {monthLabel(operational?.selectedCpsMonth ?? cpsMonth)}</strong><small>{operational?.cpsPeriodState === "mtd" ? "Month to date" : "Monthly result"}</small></div><dl><div><dt>Average CPS</dt><dd>{operational?.averageCps == null ? "—" : `₹${operational.averageCps.toFixed(2)}`}</dd></div><div><dt>On target</dt><dd>{operational?.cpsMeasured ? `${operational.cpsOnTarget}/${operational.cpsMeasured}` : "—"}</dd></div><div><dt>Measured locations</dt><dd>{operational?.cpsMeasured || "—"}</dd></div></dl></section>
      {cpsStations.length ? <section className="dx-performance-station-list dx-performance-cps-list"><header><div><small>{operational?.cpsPeriodLabel ?? monthLabel(cpsMonth)}</small><h2>CPS by location</h2></div><span>Latest {date(operational?.cpsLatestDate)}</span></header>{cpsStations.map((station) => <details key={station.id} className="dx-performance-station-card"><summary><div><i>{station.code}</i><span><strong>{station.name}</strong><small>{unitPerformance(station.unitType)} · {station.model || "Mapped location"}</small></span></div><div><Status value={station.cps?.onTarget === true ? "On target" : station.cps?.onTarget === false ? "Missed" : "Tracking"} /><b>₹{station.cps!.value.toFixed(2)}</b></div><ChevronDown /></summary><div className="dx-performance-station-detail"><div className="dx-performance-station-summary"><span><small>{operational?.cpsPeriodState === "mtd" ? "MTD CPS" : "Monthly CPS"}</small><strong>₹{station.cps!.value.toFixed(2)}</strong></span><span><small>Target</small><strong>{station.cps!.target == null ? "—" : `₹${station.cps!.target.toFixed(2)}`}</strong></span><span><small>Variance</small><strong>{station.cps!.gap == null ? "—" : `${station.cps!.gap > 0 ? "+" : ""}₹${station.cps!.gap.toFixed(2)}`}</strong></span></div><p className="dx-performance-cps-note">Updated {date(station.cps!.date)}. Cost drivers will appear here when the monthly calculation is ready.</p></div></details>)}</section> : <div className="dx-performance-empty"><CircleGauge /><strong>No CPS is available for this month</strong><span>Select another month or check after station cost calculation.</span></div>}
    </> : <>
      {data.reviews.length ? <>
        <header className="dx-performance-review-picker"><div><small>Individual review</small><h2>My goals & feedback</h2></div><label><span className="sr-only">Performance cycle</span><select value={activeReview?.id ?? ""} onChange={(event) => setActiveReviewId(event.target.value)}>{data.reviews.map((review) => <option key={review.id} value={review.id}>{cycles.get(review.cycle_id)?.name ?? "Performance review"}</option>)}</select><ChevronDown /></label></header>
      ...
      </> : <div className="dx-performance-empty"><Sparkles /><strong>No individual review yet</strong><span>Your review appears when HR launches a cycle for your People scope.</span></div>}
