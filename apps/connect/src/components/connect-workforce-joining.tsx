...
export function ConnectWorkforceJoining({account,compact=false,activationOnly=false}:{account:AppAccount;compact?:boolean;activationOnly?:boolean}) {
  ...
  return <section className="dx-dashboard-card dx-joining-card" aria-label={activationOnly?"Work setup status":"Joining status"}>
    ...
    {loading ? <p role="status">Loading your joining status…</p>:error ? <p role="alert">Joining details are unavailable. Use refresh to retry; your saved records are unchanged.</p>:data ? <>
      <div className="dx-joining-status"><strong>{data.stageLabel}</strong><span>{data.mappingEffectiveFrom ? `Work assignment effective ${date(data.mappingEffectiveFrom)}`:data.configured ? "Partner ID or commercial terms pending":"Your station team will confirm your joining plan."}</span></div>
