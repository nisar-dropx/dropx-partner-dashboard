  ...
  return <section className="dx-workforce-payments">
    ...
    {tab === "earnings" && earningsAllowed && data && !loading && !error ? <>
      {!hasMap ? <section className="dx-workforce-empty"><i><Route /></i><div><strong>Payment mapping is being set up</strong><p>Your profile is active, but it is not yet connected to a provider ID and rate card. Your station team can complete the mapping before live earnings appear here.</p></div></section> : <>
        ...
        <section className="dx-workforce-mtd-breakdown"><header><div><small>Month to date</small><h2>Payable activity break-up</h2><p>Each line follows the rate configured for this provider and role. Matching rates stay matching; they are never assumed.</p></div></header>{data.summary.rateLines.length ? <div>{data.summary.rateLines.map((line) => <article key={`${line.code}:${line.rate}`}><span><strong>{line.label}</strong><small>{line.sharedRate ? "Current shared delivery rate" : "Configured payable rate"}</small></span><b>{line.count.toLocaleString("en-IN")} × {money(line.rate)}<em>{money(line.amount)}</em></b></article>)}</div> : <div className="dx-empty"><ReceiptText /><strong>Payable rates are not published yet</strong><small>Your shipment totals are safe. This break-up appears once the rate card is configured.</small></div>}</section>
        <section className="dx-workforce-ledger"><header><div><small>Daily view</small><h2>This month&apos;s earnings</h2></div><button onClick={() => void load()} aria-label="Refresh earnings"><RefreshCw /></button></header>{data.daily.length ? <div>{data.daily.map((row) => <article className={expandedDate === row.date ? "expanded" : ""} key={row.date}><button aria-expanded={expandedDate === row.date} className="dx-workforce-day" onClick={() => setExpandedDate((current) => current === row.date ? "" : row.date)} type="button"><span><strong>{date(row.date)}</strong><small>{row.deliveries.toLocaleString("en-IN")} total deliveries · tap for rate detail</small></span><b>{money(row.earnings)}<ChevronDown /></b></button>{expandedDate === row.date ? <div className="dx-workforce-rate-breakdown">{row.rateLines.length ? row.rateLines.map((line) => <div key={`${line.code}:${line.rate}`}><span><strong>{line.label}</strong><small>{line.sharedRate ? "Uses the configured delivery rate" : "Configured rate"}</small></span><b>{line.count.toLocaleString("en-IN")} × {money(line.rate)}<em>{money(line.amount)}</em></b></div>) : <small>Shipment counts are available; the rate card has not published a payable rate for this day yet.</small>}</div> : null}</article>)}</div> : <div className="dx-empty"><ReceiptText /><strong>No imported delivery data yet</strong><small>New Amazon delivery imports will show here after they are mapped and processed.</small></div>}</section>
