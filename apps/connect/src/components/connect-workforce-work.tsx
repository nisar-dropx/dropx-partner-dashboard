...
export function ConnectWorkforceWork({ account, initial = "roster" }: { account: AppAccount; initial?: Tab }) {
  ...
  const tabs = ([
    access.includes("roster") ? ["roster", "Pincode & weekly off", <ArrowLeftRight key="roster" />] : null,
  ...
  return <section className="dx-workforce-work"><header className="dx-page-intro"><small>My schedule</small><h1>Work schedule</h1><p>Check your operating pincode and weekly off, request time off and view attendance.</p></header><nav aria-label="Work section" className="dx-workforce-tabs">{tabs.map(([value, name, icon]) => <button className={tab === value ? "active" : ""} key={value} onClick={() => setTab(value)}>{icon}{name}</button>)}</nav>{tab === "attendance" ? <ConnectAttendance account={account} /> : null}{tab === "roster" ? <ConnectRoster account={account} /> : null}{tab === "leave" ? <ConnectLeave account={account} /> : null}</section>;
}
