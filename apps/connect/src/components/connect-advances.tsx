  ...
  return <section className="dx-advances">
    ...
    {loading ? <div className="dx-loader"><span /><small>Loading advances...</small></div> : rows.length ? <div className="dx-advance-list">
      {rows.map((row) => <article key={row.id}>
        ...
        <p>{row.purpose}</p>
        <dl><div><dt>Requested on</dt><dd>{new Date(row.requested_at).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}</dd></div>{row.approved_amount != null ? <div><dt>Approved amount</dt><dd>{money(Number(row.approved_amount))}</dd></div> : null}{row.decision_comment ? <div><dt>Comment</dt><dd>{row.decision_comment}</dd></div> : null}<div><dt>Updated on</dt><dd>{new Date(row.updated_at).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}</dd></div></dl>
      </article>)}
