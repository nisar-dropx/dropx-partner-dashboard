...
import { workforcePaymentStatus, workforceStatementDate as date } from "@/lib/workforce-payment-status";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!account.pageAccess.includes("earnings")) return new NextResponse("Payments are not enabled for this account",{status:403});
    const item = await supabaseAdmin.from("workforce_payroll_items")
      .select("id,status,payroll_run_id,worker_name,dropx_id,station_code,shipment_count,work_days,base_amount,incentive_amount,adjustment_amount,deduction_amount,gross_amount,net_amount,workforce_payroll_runs!inner(run_number,period_start,period_end,status,payment_reference,payment_date,paid_at)")
